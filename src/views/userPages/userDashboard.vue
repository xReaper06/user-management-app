    ...
    <div>
      <div class="card">
        <div class="card-body">
          ...
          <!-- My Task section -->
          <div class="fs-3 text-center"><strong>My Task</strong>
        date
         <input type="date" id="timenow" name="timenow" v-model="timenow" @change="getTask"/></div>
          <hr>
  
          <!-- Task cards loop -->
          <div v-for="task in myTask" :key="task.id" class="mb-4">
                <div class="" :class="task.is_done == 0 ? 'card custom-card': 'card custom-card done'">
                    <div v-if="overWork(task.time_duration)" class="card-header bg-danger">OverWork !!!</div>
                  <div class="card-body">
                    <div class="d-flex flex-column flex-md-row align-items-center">
                      ...
                      <div class="ms-md-5">
                        <div v-if="task.is_done == 0">
                            <button type="button" class="btn btn-primary me-3" @click="doneTask(task.id)">
                              Done
                            </button>
                        </div>
                        <button type="button" class="btn btn-danger" @click="removeTask(task.id)">
                          Remove
                        </button>
                      </div>
                    </div>
...
function getCurrentTime12Hour() {
  ...
  // Format the time in 12-hour format
  const currentTime12Hour = `${hours12}:${padZero(minutes)}:${padZero(seconds)} ${period}`;
...
function getCurrentTimeNoSeconds() {
  ...
  const seconds = now.getSeconds();
  // Format the time in 12-hour format
  const currentTime12Hour = `${padZero(hours)}:${padZero(minutes)}:${padZero(seconds)}`;
